...
Usage:
    python train_from_notebook_14_clean.py
"""
...
# CRITICAL FIX: Set OpenCV environment variables BEFORE importing cv2
# This prevents video loading deadlocks on Windows
...
import pandas as pd
import cv2
cv2.setNumThreads(0)  # Disable OpenCV internal threading to prevent deadlocks
...
class EnhancedMultimodalDataset(Dataset):
    ...
    def _load_audio(self, path):
        try:
            # Add timeout protection and resource cleanup
            import warnings
            warnings.filterwarnings('ignore')
            
            waveform, sr = librosa.load(path, sr=self.config.sample_rate, duration=10, res_type='kaiser_fast')
    ...
    def _load_video_frame(self, path):
        # Skip non-video files immediately (e.g., .npz files)
        if path.endswith('.npz'):
            return self._load_youtube_npz(path)
        ...
        try:
            ...
            # Ensure cap is released even if exception occurs
            if cap is not None:
                try:
                    cap.release()
                except:
    ...
    def _load_youtube_npz(self, path):
        ...
        try:
            ...
            pass  # Silently fail and return zeros
...
print("✅ YouTube Faces .npz handler integrated into dataset class!")
...
# Create DataLoaders with WINDOWS OPTIMIZATION + ANTI-FREEZE FIX
train_loader = DataLoader(
    ...
    drop_last=True,     # Drop incomplete batches to prevent errors
    timeout=0,  # No timeout for num_workers=0
